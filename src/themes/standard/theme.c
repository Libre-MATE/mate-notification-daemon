...
static void create_border_with_arrow(GtkWidget* nw, WindowData* windata) {
    ...
    case GTK_ARROW_DOWN:
      ...
      if (norm_point_x < arrow_side1_width) {
        ...
        if (norm_point_x - arrow_side2_width + width >=
          ...
          arrow_offset =
              MIN(norm_point_x - arrow_side1_width, DEFAULT_ARROW_OFFSET);
        }
      ...
      windata->drawn_arrow_end_x =
          arrow_offset + arrow_side1_width + arrow_side2_width;
      ...
      if (arrow_type == GTK_ARROW_UP) {
        ...
        if (arrow_side2_width > 0) {
          ADD_POINT(windata->drawn_arrow_end_x, windata->drawn_arrow_end_y, 1,
                    0);
        ...
        if (arrow_side2_width == 0) {
          ...
          if (arrow_offset < width - arrow_side1_width) {
            ADD_POINT(arrow_offset + arrow_side1_width + arrow_side2_width,
        ...
        if (arrow_side1_width > 0) {
          ADD_POINT(
              windata->drawn_arrow_begin_x - (arrow_side2_width > 0 ? 0 : 1),
  ...
  /*windata->window_region = gdk_region_polygon(shape_points,
   * windata->num_border_points, GDK_EVEN_ODD_RULE);*/
  g_free(shape_points);
}
...
static void draw_border(GtkWidget* widget, WindowData* windata, cairo_t* cr) {
  ...
  if (windata->has_arrow) {
    ...
    cairo_move_to(cr, windata->border_points[0].x + 0.5,
                  windata->border_points[0].y + 0.5);

    for (i = 1; i < windata->num_border_points; i++) {
      cairo_line_to(cr, windata->border_points[i].x + 0.5,
                    windata->border_points[i].y + 0.5);
    }

    cairo_close_path(cr);
    /* FIXME window_region is not set up anyway, see previous fixme */
    /*gdk_window_shape_combine_region (gtk_widget_get_window (windata->win),
...
static void paint_countdown(GtkWidget* pie, cairo_t* cr, WindowData* windata) {
  ...
  if (windata->timeout > 0) {
    ...
    cairo_arc_negative(cr2, PIE_RADIUS, PIE_RADIUS, PIE_RADIUS, -G_PI_2,
                       -(pct * G_PI * 2) - G_PI_2);
...
static gboolean on_countdown_draw(GtkWidget* widget, cairo_t* cr,
                                  WindowData* windata) {
