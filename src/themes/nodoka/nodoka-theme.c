...
static gboolean activate_link(GtkLabel *label, const char *url,
                              WindowData *windata) {
  windata->url_clicked(GTK_WINDOW(windata->win), url);
  return TRUE;
}

static void get_size_of_widgets_monitor(GtkWidget *widget, int *width,
                                        int *height) {
...
static void set_arrow_parameters(WindowData *windata) {
  ...
  if ((windata->arrow.position.x - DEFAULT_ARROW_SKEW - DEFAULT_ARROW_OFFSET +
       alloc.width) > monitor_width) {
    windata->arrow.offset = windata->arrow.position.x - DEFAULT_ARROW_SKEW -
                            (monitor_width - alloc.width);
  ...
  } else if (windata->arrow.offset + DEFAULT_ARROW_WIDTH + 6 > alloc.width) {
    windata->arrow.offset = alloc.width - 6 - DEFAULT_ARROW_WIDTH;
    windata->arrow.position.x -= 6;
  }

  /* Set arrow points X position */
  windata->arrow.point_begin.x = windata->arrow.offset;
  windata->arrow.point_middle.x = windata->arrow.offset + DEFAULT_ARROW_SKEW;
  windata->arrow.point_end.x = windata->arrow.offset + DEFAULT_ARROW_WIDTH;
  ...
  switch (arrow_type) {
    case GTK_ARROW_UP:
      ...
      break;
    case GTK_ARROW_DOWN:
      windata->arrow.point_begin.y = alloc.height - DEFAULT_ARROW_HEIGHT;
      windata->arrow.point_middle.y = alloc.height;
      windata->arrow.point_end.y = alloc.height - DEFAULT_ARROW_HEIGHT;
...
static void destroy_windata(WindowData *windata) { g_free(windata); }
...
static void nodoka_rounded_rectangle(cairo_t *cr, double x, double y, double w,
                                     double h, int radius) {
...
static void nodoka_rounded_rectangle_with_arrow(cairo_t *cr, double x, double y,
                                                double w, double h, int radius,
                                                ArrowParameters *arrow) {
  gboolean arrow_up;
  ...
  if (arrow_up) {
    cairo_line_to(cr, rect.x + arrow->point_begin.x, rect.y);
    cairo_line_to(cr, rect.x + arrow->point_middle.x,
                  rect.y - DEFAULT_ARROW_HEIGHT);
    cairo_line_to(cr, rect.x + arrow->point_end.x, rect.y);
  }

  cairo_arc(cr, rect.x + rect.width - radius, rect.y + radius, radius,
            G_PI * 1.5, G_PI * 2);
  cairo_arc(cr, rect.x + rect.width - radius, rect.y + rect.height - radius,
            radius, 0, G_PI * 0.5);

  if (!arrow_up) {
    cairo_line_to(cr, rect.x + arrow->point_end.x, rect.y + rect.height);
    cairo_line_to(cr, rect.x + arrow->point_middle.x,
                  rect.y + rect.height + DEFAULT_ARROW_HEIGHT);
    cairo_line_to(cr, rect.x + arrow->point_begin.x, rect.y + rect.height);
  }

  cairo_arc(cr, rect.x + radius, rect.y + rect.height - radius, radius,
            G_PI * 0.5, G_PI);
  cairo_arc(cr, rect.x + radius, rect.y + radius, radius, G_PI, G_PI * 1.5);

  cairo_translate(cr, -x, -y);
}

/* Fill background */
static void fill_background(GtkWidget *widget, WindowData *windata,
                            cairo_t *cr) {
  double alpha;
  ...
  cairo_pattern_add_color_stop_rgba(pattern, GRADIENT_CENTER, 0.988, 0.988,
                                    0.714, alpha);
  ...
  if (windata->arrow.has_arrow)
    nodoka_rounded_rectangle_with_arrow(cr, 0, 0, windata->width,
                                        windata->height, 6, &(windata->arrow));
  else
...
static void draw_stripe(GtkWidget *widget, WindowData *windata, cairo_t *cr) {
  ...
  cairo_pattern_t *pattern;
  pattern = cairo_pattern_create_linear(0, 0, 0, windata->height);
  cairo_pattern_add_color_stop_rgba(pattern, 0, top_color.red / color_mult,
                                    top_color.green / color_mult,
                                    top_color.blue / color_mult, alpha);
  cairo_pattern_add_color_stop_rgba(
      pattern, GRADIENT_CENTER, bottom_color.red / color_mult,
      bottom_color.green / color_mult, bottom_color.blue / color_mult, alpha);
  cairo_pattern_add_color_stop_rgba(pattern, 1, bottom_color.red / color_mult,
                                    bottom_color.green / color_mult,
                                    bottom_color.blue / color_mult, alpha);
  cairo_set_source(cr, pattern);
  cairo_pattern_destroy(pattern);

  if (windata->arrow.has_arrow)
    nodoka_rounded_rectangle_with_arrow(cr, 1, 1, windata->width - 2,
                                        windata->height - 2, 5,
                                        &(windata->arrow));
  else
    nodoka_rounded_rectangle(cr, 1, 1, windata->width - 2, windata->height - 2,
                             5);
  cairo_fill(cr);
...
static void draw_border(GtkWidget *widget, WindowData *windata, cairo_t *cr) {
  ...
  if (windata->arrow.has_arrow)
    nodoka_rounded_rectangle_with_arrow(cr, 0.5, 0.5, windata->width - 1,
                                        windata->height - 1, 6,
                                        &(windata->arrow));
  else
...
static void draw_pie(GtkWidget *pie, WindowData *windata, cairo_t *cr) {
  if (windata->timeout == 0) return;

  gdouble arc_angle =
      1.0 - (gdouble)windata->remaining / (gdouble)windata->timeout;
  cairo_set_source_rgba(cr, 1.0, 0.4, 0.0, 0.3);
  cairo_move_to(cr, PIE_RADIUS, PIE_RADIUS);
  cairo_arc_negative(cr, PIE_RADIUS, PIE_RADIUS, PIE_RADIUS, -G_PI / 2,
                     (-0.25 + arc_angle) * 2 * G_PI);
...
static void paint_window(GtkWidget *widget, cairo_t *cr, WindowData *windata) {
  ...
  if (windata->arrow.has_arrow) set_arrow_parameters(windata);
  ...
  if (windata->arrow.has_arrow) {
    nodoka_rounded_rectangle_with_arrow(cr2, 0, 0, windata->width,
                                        windata->height, 6, &(windata->arrow));
  } else {
...
/* Event handlers */
static gboolean configure_event_cb(GtkWidget *nw, GdkEventConfigure *event,
                                   WindowData *windata) {
...
static gboolean countdown_expose_cb(GtkWidget *pie, cairo_t *cr,
                                    WindowData *windata) {
  cairo_t *cr2;
  cairo_surface_t *surface;
  GtkAllocation alloc;
  ...
  surface = cairo_surface_create_similar(cairo_get_target(cr),
                                         CAIRO_CONTENT_COLOR_ALPHA, alloc.width,
                                         alloc.height);
  cr2 = cairo_create(surface);
...
static void action_clicked_cb(GtkWidget *w, GdkEventButton *event,
...
GtkWindow *create_notification(UrlClickedCb url_clicked) {
  ...
  if (visual != NULL) {
    ...
    if (gdk_screen_is_composited(screen)) windata->composited = TRUE;
  }
  ...
  g_signal_connect(G_OBJECT(win), "composited-changed",
                   G_CALLBACK(on_composited_changed), windata);
  ...
  gtk_box_pack_start(GTK_BOX(main_vbox), windata->bottom_spacer, FALSE, FALSE,
                     0);
  ...
  gtk_label_set_line_wrap_mode(GTK_LABEL(windata->summary_label),
  ...
  gtk_box_pack_start(GTK_BOX(windata->content_hbox), windata->iconbox, FALSE,
                     FALSE, 0);
...
void set_notification_text(GtkWindow *nw, const char *summary,
  ...
  str = g_strdup_printf("<span color=\"#000000\"><b><big>%s</big></b></span>",
                        quoted);
  g_free(quoted);
  ...
  xmlDocPtr doc;
  xmlInitParser();
  str = g_strconcat("<markup>", "<span color=\"#000000\">", body, "</span>",
                    "</markup>", NULL);
...
render_ok:
  ...
  gtk_widget_set_size_request(
      ((body != NULL && *body == '\0') ? windata->body_label
                                       : windata->summary_label),
      WIDTH - (IMAGE_SIZE + IMAGE_PADDING) - 10, -1);
}
...
void notification_tick(GtkWindow *nw, glong remaining) {
  ...
  if (windata->pie_countdown != NULL) {
    gtk_widget_queue_draw_area(windata->pie_countdown, 0, 0, PIE_WIDTH,
                               PIE_HEIGHT);
  }
}
