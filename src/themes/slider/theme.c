...
static void draw_round_rect(cairo_t* cr, gdouble aspect, gdouble x, gdouble y,
                            gdouble corner_radius, gdouble width,
                            gdouble height) {
  ...
  cairo_arc(cr, x + width - radius, y + radius, radius, -90.0f * G_PI / 180.0f,
            0.0f * G_PI / 180.0f);
  ...
  cairo_arc(cr, x + radius, y + height - radius, radius, 90.0f * G_PI / 180.0f,
            180.0f * G_PI / 180.0f);
  ...
  cairo_arc(cr, x + radius, y + radius, radius, 180.0f * G_PI / 180.0f,
            270.0f * G_PI / 180.0f);
}
...
static void fill_background(GtkWidget* widget, WindowData* windata,
  ...
  draw_round_rect(cr, 1.0f, DEFAULT_X0 + 1, DEFAULT_Y0 + 1, DEFAULT_RADIUS,
...
static gboolean on_window_map(GtkWidget* widget, GdkEvent* event,
                              WindowData* windata) {
  return FALSE;
}
...
GtkWindow* create_notification(UrlClickedCb url_clicked) {
  ...
  g_signal_connect(win, "composited-changed", G_CALLBACK(on_composited_changed),
                   windata);
  ...
  gtk_box_pack_start(GTK_BOX(windata->main_hbox), windata->icon, FALSE, FALSE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(windata->main_hbox), windata->close_button, FALSE,
                     FALSE, 0);
...
void set_notification_icon(GtkWindow* nw, GdkPixbuf* pixbuf) {
  ...
  if (scaled != NULL) {
    ...
    gtk_widget_show(windata->icon);
    gtk_widget_set_size_request(windata->icon, MAX(BODY_X_OFFSET, pixbuf_width),
                                -1);
...
static void on_action_clicked(GtkWidget* w, GdkEventButton* event,
...
gboolean get_always_stack(GtkWidget* nw) { return TRUE; }
