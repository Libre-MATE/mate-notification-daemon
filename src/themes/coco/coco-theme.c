...
static void fill_background(GtkWidget *widget, WindowData *windata,
  ...
  cairo_pattern_add_color_stop_rgba(pattern, 0, 19 / 255.0, 19 / 255.0,
                                    19 / 255.0, alpha);
  cairo_pattern_add_color_stop_rgba(pattern, GRADIENT_CENTER, 19 / 255.0,
                                    19 / 255.0, 19 / 255.0, alpha);
  cairo_pattern_add_color_stop_rgba(pattern, 1, 19 / 255.0, 19 / 255.0,
                                    19 / 255.0, alpha);
  cairo_set_source(cr, pattern);
  cairo_pattern_destroy(pattern);

  nodoka_rounded_rectangle(cr, 0, 8, windata->width - 8, windata->height - 8,
                           6);
...
GtkWindow *create_notification(UrlClickedCb url_clicked) {
  ...
  gtk_box_pack_start(GTK_BOX(windata->main_hbox), windata->iconbox, FALSE,
                     FALSE, 0);

  windata->icon = gtk_image_new();
  gtk_box_pack_start(GTK_BOX(windata->iconbox), windata->icon, FALSE, FALSE, 0);
...
void set_notification_text(GtkWindow *nw, const char *summary,
  ...
  str = g_strdup_printf("<span color=\"#FFFFFF\"><big><b>%s</b></big></span>",
                        quoted);
  g_free(quoted);
  ...
  xmlDocPtr doc;
  xmlInitParser();
  str = g_strconcat("<markup>", "<span color=\"#EAEAEA\">", body, "</span>",
                    "</markup>", NULL);
