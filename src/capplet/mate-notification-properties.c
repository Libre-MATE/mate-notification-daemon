...
enum { NOTIFY_POSITION_LABEL, NOTIFY_POSITION_NAME, N_COLUMNS_POSITION };

enum { NOTIFY_MONITOR_NUMBER, N_COLUMNS_MONITOR };
...
static void notification_properties_position_notify(
    GSettings* settings, gchar* key, NotificationAppletDialog* dialog) {
  GtkTreeModel* model;
  GtkTreeIter iter;
  char* location;
  gboolean valid;
...
static void notification_properties_monitor_changed(
    GtkComboBox* widget, NotificationAppletDialog* dialog) {
  gint monitor;
  GtkTreeIter iter;

  GtkTreeModel* model =
      gtk_combo_box_get_model(GTK_COMBO_BOX(dialog->monitor_combo));

  if (!gtk_combo_box_get_active_iter(GTK_COMBO_BOX(dialog->monitor_combo),
                                     &iter)) {
    return;
  }
...
static void notification_properties_monitor_notify(
    GSettings* settings, gchar* key, NotificationAppletDialog* dialog) {
  GtkTreeModel* model;
  GtkTreeIter iter;
  gint monitor_number;
  ...
  monitor_number =
      g_settings_get_int(dialog->gsettings, GSETTINGS_KEY_MONITOR_NUMBER);

  for (valid = gtk_tree_model_get_iter_first(model, &iter); valid;
       valid = gtk_tree_model_iter_next(model, &iter)) {
    gtk_tree_model_get(model, &iter, NOTIFY_MONITOR_NUMBER,
                       &monitor_number_at_iter, -1);

    if (monitor_number_at_iter == monitor_number) {
      gtk_combo_box_set_active_iter(GTK_COMBO_BOX(dialog->monitor_combo),
                                    &iter);
      break;
    }
  }
...
static void notification_properties_location_changed(
    GtkComboBox* widget, NotificationAppletDialog* dialog) {
  char* location;
  GtkTreeIter iter;

  GtkTreeModel* model =
      gtk_combo_box_get_model(GTK_COMBO_BOX(dialog->position_combo));
  ...
  g_settings_set_string(dialog->gsettings, GSETTINGS_KEY_POPUP_LOCATION,
                        location);
  g_free(location);
}

static void notification_properties_dialog_setup_positions(
  ...
  g_signal_connect(dialog->gsettings, "changed::" GSETTINGS_KEY_POPUP_LOCATION,
                   G_CALLBACK(notification_properties_position_notify), dialog);
  ...
  model = gtk_combo_box_get_model(GTK_COMBO_BOX(dialog->position_combo));
  location =
      g_settings_get_string(dialog->gsettings, GSETTINGS_KEY_POPUP_LOCATION);
...
static void notification_properties_dialog_setup_monitors(
    NotificationAppletDialog* dialog) {
  GtkListStore* store;
  GdkDisplay* display;
  GdkScreen* screen;
  ...
  gtk_combo_box_set_model(GTK_COMBO_BOX(dialog->monitor_combo),
                          GTK_TREE_MODEL(store));

  g_signal_connect(dialog->gsettings, "changed::" GSETTINGS_KEY_MONITOR_NUMBER,
                   G_CALLBACK(notification_properties_monitor_notify), dialog);
  cur_monitor_number =
      g_settings_get_int(dialog->gsettings, GSETTINGS_KEY_MONITOR_NUMBER);
  ...
  for (valid = gtk_tree_model_get_iter_first(GTK_TREE_MODEL(store), &iter);
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(store), &iter, NOTIFY_MONITOR_NUMBER,
                       &cur_monitor_number_at_iter, -1);
  ...
  g_signal_connect(dialog->monitor_combo, "changed",
                   G_CALLBACK(notification_properties_monitor_changed), dialog);
}

static void notification_properties_theme_notify(
    GSettings* settings, gchar* key, NotificationAppletDialog* dialog) {
  char* theme = g_settings_get_string(dialog->gsettings, key);

  GtkTreeModel* model =
      gtk_combo_box_get_model(GTK_COMBO_BOX(dialog->theme_combo));
...
static void notification_properties_theme_changed(
    GtkComboBox* widget, NotificationAppletDialog* dialog) {
  char* theme;
  GtkTreeIter iter;
...
static void notification_properties_dialog_setup_themes(
  ...
  g_signal_connect(dialog->gsettings, "changed::" GSETTINGS_KEY_THEME,
                   G_CALLBACK(notification_properties_theme_notify), dialog);
  ...
  if ((dir = g_dir_open(ENGINES_DIR, 0, NULL))) {
    while ((filename = g_dir_read_name(dir))) {
      if (g_str_has_suffix(filename, "." G_MODULE_SUFFIX)) {
        ...
        gtk_list_store_append(store, &iter);
        gtk_list_store_set(store, &iter, NOTIFY_THEME_LABEL, theme_label,
                           NOTIFY_THEME_NAME, theme_name, NOTIFY_THEME_FILENAME,
                           filename, -1);
        g_free(theme_name);
        g_free(theme_label);
      }
  ...
  for (valid = gtk_tree_model_get_iter_first(GTK_TREE_MODEL(store), &iter);
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(store), &iter, NOTIFY_THEME_NAME, &key,
                       -1);
...
static void show_message(NotificationAppletDialog* dialog,
                         const gchar* message) {
  GtkWidget* d = gtk_message_dialog_new(
      GTK_WINDOW(dialog->dialog), GTK_DIALOG_DESTROY_WITH_PARENT,
      GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "%s", message);
  gtk_dialog_run(GTK_DIALOG(d));
  gtk_widget_destroy(d);
}

static void notification_properties_dialog_preview_closed(
    NotifyNotification* preview, NotificationAppletDialog* dialog) {
...
static void notification_properties_dialog_preview(
    NotificationAppletDialog* dialog) {
  ...
  dialog->preview = notify_notification_new(
      _("Notification Test"), _("Just a test"), "dialog-information");
...
static void notification_properties_dialog_response(
    GtkWidget* widget, int response, NotificationAppletDialog* dialog) {
  switch (response) {
...
static void notification_properties_dialog_destroyed(
    GtkWidget* widget, NotificationAppletDialog* dialog) {
  dialog->dialog = NULL;

  gtk_main_quit();
}

static gboolean notification_properties_dialog_init(
    NotificationAppletDialog* dialog) {
  ...
  gtk_builder_add_from_resource(
      builder,
      "/org/mate/notifications/properties/mate-notification-properties.ui",
      &error);
  if (error != NULL) {
  ...
  dialog->position_combo =
      GTK_WIDGET(gtk_builder_get_object(builder, "position_combo"));
  dialog->monitor_combo =
      GTK_WIDGET(gtk_builder_get_object(builder, "monitor_combo"));
  dialog->theme_combo =
      GTK_WIDGET(gtk_builder_get_object(builder, "theme_combo"));
  dialog->active_checkbox =
      GTK_WIDGET(gtk_builder_get_object(builder, "use_active_check"));
  dialog->dnd_checkbox =
      GTK_WIDGET(gtk_builder_get_object(builder, "do_not_disturb_check"));
  dialog->monitor_label =
      GTK_WIDGET(gtk_builder_get_object(builder, "monitor_label"));
  ...
  g_signal_connect(dialog->dialog, "response",
                   G_CALLBACK(notification_properties_dialog_response), dialog);
  g_signal_connect(dialog->dialog, "destroy",
                   G_CALLBACK(notification_properties_dialog_destroyed),
                   dialog);

  g_settings_bind(dialog->gsettings, GSETTINGS_KEY_USE_ACTIVE_MONITOR,
                  dialog->active_checkbox, "active", G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(dialog->gsettings, GSETTINGS_KEY_DO_NOT_DISTURB,
                  dialog->dnd_checkbox, "active", G_SETTINGS_BIND_DEFAULT);
  ...
  inv_active_checkbox =
      !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dialog->active_checkbox));
  ...
  g_object_bind_property(dialog->active_checkbox, "active",
                         dialog->monitor_combo, "sensitive",
                         G_BINDING_INVERT_BOOLEAN);
  g_object_bind_property(dialog->active_checkbox, "active",
                         dialog->monitor_label, "sensitive",
                         G_BINDING_INVERT_BOOLEAN);
...
static void notification_properties_dialog_finalize(
    NotificationAppletDialog* dialog) {
