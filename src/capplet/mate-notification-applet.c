...
static void show_about(GtkAction *action, MateNotificationApplet *applet);
static void call_properties(GtkAction *action, MateNotificationApplet *applet);

static const GtkActionEntry applet_menu_actions[] = {
    {"Preferences", "document-properties", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(call_properties)},
    {"About", "help-about", N_("_About"), NULL, NULL, G_CALLBACK(show_about)}};

static const char *notification_properties[] = {
    "mate-notification-properties",
};

static void applet_destroy(MatePanelApplet *applet_widget,
  ...
  for (i = 0; i < G_N_ELEMENTS(notification_properties); i++) {
    ...
    if (programpath != NULL) {
      ...
      mate_gdk_spawn_command_line_on_screen(
          ...
          notification_properties[i], NULL);
      return;
    }
  }
}
...
static void show_about(GtkAction *action, MateNotificationApplet *applet) {
  static const char *authors[] = {"Robert Buj <robert.buj@gmail.com>", NULL};
  (void)action;
  (void)applet;
...
static void set_status_image(MateNotificationApplet *applet, gboolean active) {
...
static void settings_changed(GSettings *settings, gchar *key,
                             MateNotificationApplet *applet) {
  if (g_strcmp0(GSETTINGS_KEY_DO_NOT_DISTURB, key) == 0)
    set_status_image(applet, g_settings_get_boolean(settings, key));
}

static void applet_draw_icon(MatePanelApplet *applet_widget, int arg1,
                             MateNotificationApplet *applet) {
  gint size, scale;
  ...
  cairo_surface_t *image_on =
      gtk_icon_theme_load_surface(gtk_icon_theme_get_default(),
                                  "user-available", size, scale, NULL, 0, NULL);
  cairo_surface_t *image_off =
      gtk_icon_theme_load_surface(gtk_icon_theme_get_default(),
                                  "user-invisible", size, scale, NULL, 0, NULL);
...
static MateNotificationApplet *applet_main(MatePanelApplet *applet_widget) {
  MateNotificationApplet *applet;
  GtkWidget *box;
  ...
  set_status_image(applet, g_settings_get_boolean(
                               applet->settings, GSETTINGS_KEY_DO_NOT_DISTURB));
  ...
  mate_panel_applet_setup_menu_from_resource(
      applet->applet, RESOURCE_PATH "menu.xml", applet->action_group);
...
MATE_PANEL_APPLET_OUT_PROCESS_FACTORY("MateNotificationAppletFactory",
                                      PANEL_TYPE_APPLET,
                                      "Do Not Disturb Applet", applet_factory,
                                      NULL)
