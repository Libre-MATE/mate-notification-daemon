...
GList *notify_stack_get_windows(NotifyStack *stack) { return stack->windows; }

#ifdef HAVE_X11
static gboolean get_work_area(NotifyStack *stack, GdkRectangle *rect) {
  Atom workarea;
  Atom type;
  Window win;
  int format;
  ...
  workarea = XInternAtom(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                         "_NET_WORKAREA", True);
  ...
  win =
      XRootWindow(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), disp_screen);

  result = XGetWindowProperty(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                              win, workarea, 0, max_len, False, AnyPropertyType,
                              &type, &format, &num, &leftovers, &ret_workarea);
...
static void get_origin_coordinates(NotifyStackLocation stack_location,
                                   GdkRectangle *workarea, gint *x, gint *y,
                                   gint *shiftx, gint *shifty, gint width,
                                   gint height) {
...
static void translate_coordinates(NotifyStackLocation stack_location,
                                  GdkRectangle *workarea, gint *x, gint *y,
                                  gint *shiftx, gint *shifty, gint width,
                                  gint height) {
  switch (stack_location) {
...
NotifyStack *notify_stack_new(NotifyDaemon *daemon, GdkScreen *screen,
  ...
  g_assert((guint)_gtk_get_monitor_num(monitor) <
           (guint)gdk_display_get_n_monitors(display));
...
static void notify_stack_shift_notifications(NotifyStack *stack, GtkWindow *nw,
                                             GList **nw_l, gint init_width,
                                             gint init_height, gint *nw_x,
                                             gint *nw_y) {
  ...
  get_origin_coordinates(stack->location, &workarea, &x, &y, &shiftx, &shifty,
  ...
  for (i = 0, l = stack->windows; l != NULL; i++, l = l->next) {
    ...
    if (nw == NULL || nw2 != nw) {
      ...
      translate_coordinates(stack->location, &workarea, &x, &y, &shiftx,
                            &shifty, req.width,
  ...
  for (i = n_wins - 1, l = g_list_last(stack->windows); l != NULL;
       i--, l = l->prev) {
    GtkWindow *nw2 = GTK_WINDOW(l->data);
...
static void update_position(NotifyStack *stack) {
  /* notify_stack_shift_notifications(stack, window, list pointer, init width,
   * init height, out window x, out window y) */
...
void notify_stack_add_window(NotifyStack *stack, GtkWindow *nw,
  ...
  notify_stack_shift_notifications(stack, nw, NULL, req.width,
                                   req.height + NOTIFY_STACK_SPACING, &x, &y);
