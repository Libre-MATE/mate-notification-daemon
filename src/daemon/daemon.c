...
#define NW_GET_DAEMON(nw) (g_object_get_data(G_OBJECT(nw), "_notify_daemon"))

enum { PROP_0, PROP_REPLACE, LAST_PROP };
...
static NotifyStackLocation get_stack_location_from_string(
    const gchar* slocation);

#ifdef HAVE_X11
static GdkFilterReturn _notify_x11_filter(GdkXEvent* xevent, GdkEvent* event,
                                          NotifyDaemon* daemon);
...
static gboolean notify_daemon_notify_handler(
    NotifyDaemonNotifications* object, GDBusMethodInvocation* invocation,
    const gchar* app_name, guint id, const gchar* icon, const gchar* summary,
    const gchar* body, const gchar* const* actions, GVariant* hints,
    gint timeout, gpointer user_data);
static gboolean notify_daemon_close_notification_handler(
    NotifyDaemonNotifications* object, GDBusMethodInvocation* invocation,
    guint arg_id, gpointer user_data);
static gboolean notify_daemon_get_capabilities(
    NotifyDaemonNotifications* object, GDBusMethodInvocation* invocation);
static gboolean notify_daemon_get_server_information(
    NotifyDaemonNotifications* object, GDBusMethodInvocation* invocation,
    gpointer user_data);
...
static void bus_acquired_handler_cb(GDBusConnection* connection,
  ...
  g_signal_connect(daemon->skeleton, "handle-notify",
                   G_CALLBACK(notify_daemon_notify_handler), daemon);
  g_signal_connect(daemon->skeleton, "handle-close-notification",
                   G_CALLBACK(notify_daemon_close_notification_handler),
                   daemon);
  g_signal_connect(daemon->skeleton, "handle-get-capabilities",
                   G_CALLBACK(notify_daemon_get_capabilities), daemon);
  g_signal_connect(daemon->skeleton, "handle-get-server-information",
                   G_CALLBACK(notify_daemon_get_server_information), daemon);

  exported = g_dbus_interface_skeleton_export(
      G_DBUS_INTERFACE_SKELETON(daemon->skeleton), connection,
      NOTIFICATION_BUS_PATH, &error);
  if (!exported) {
...
static void notify_daemon_constructed(GObject* object) {
  ...
  if (daemon->replace) flags |= G_BUS_NAME_OWNER_FLAGS_REPLACE;

  daemon->bus_name_id = g_bus_own_name(
      G_BUS_TYPE_SESSION, NOTIFICATION_BUS_NAME, flags, bus_acquired_handler_cb,
      NULL, name_lost_handler_cb, daemon, NULL);
}

static void notify_daemon_set_property(GObject* object, guint prop_id,
...
static void add_exit_timeout(NotifyDaemon* daemon) {
  ...
  if (daemon->exit_timeout_source > 0) return;

  daemon->exit_timeout_source =
      g_timeout_add_seconds(IDLE_SECONDS, do_exit, NULL);
}
...
static void on_screen_monitors_changed(GdkScreen* screen,
                                       NotifyDaemon* daemon) {
  GdkDisplay* display;
...
static void create_screen(NotifyDaemon* daemon) {
  ...
  g_signal_connect(screen, "monitors-changed",
                   G_CALLBACK(on_screen_monitors_changed), daemon);
  ...
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
    ...
    daemon->screen->workarea_atom =
        XInternAtom(GDK_DISPLAY_XDISPLAY(display), "_NET_WORKAREA", True);
...
static void on_popup_location_changed(GSettings* settings, gchar* key,
                                      NotifyDaemon* daemon) {
  NotifyStackLocation stack_location;
  gchar* slocation;
  gsize i;
  ...
  if (slocation != NULL && *slocation != '\0') {
    ...
    g_settings_set_string(
        daemon->gsettings, GSETTINGS_KEY_POPUP_LOCATION,
...
static void notify_daemon_init(NotifyDaemon* daemon) {
  ...
  g_signal_connect(daemon->gsettings, "changed::" GSETTINGS_KEY_POPUP_LOCATION,
                   G_CALLBACK(on_popup_location_changed), daemon);

  location =
      g_settings_get_string(daemon->gsettings, GSETTINGS_KEY_POPUP_LOCATION);
  ...
  daemon->notification_hash = g_hash_table_new_full(
      g_int_hash, g_int_equal, g_free, (GDestroyNotify)_notify_timeout_destroy);
}
...
static void destroy_screen(NotifyDaemon* daemon) {
  ...
  g_signal_handlers_disconnect_by_func(
      screen, G_CALLBACK(on_screen_monitors_changed), daemon);
...
static NotifyStackLocation get_stack_location_from_string(
  ...
  if (slocation == NULL || *slocation == '\0') {
    ...
    for (l = popup_stack_locations; l->type != NOTIFY_STACK_LOCATION_UNKNOWN;
         l++) {
...
static void _emit_closed_signal(GtkWindow* nw, NotifydClosedReason reason) {
  ...
  notify_daemon_notifications_emit_notification_closed(daemon->skeleton, id,
                                                       reason);
}
...
static gboolean idle_reposition_notification(IdleRepositionData* data) {
  ...
  nt = (NotifyTimeout*)g_hash_table_lookup(daemon->notification_hash,
                                           &notify_id);
  ...
  g_hash_table_remove(daemon->idle_reposition_notify_ids,
...
static void _queue_idle_reposition_notification(NotifyDaemon* daemon,
                                                gint notify_id) {
  ...
  if (g_hash_table_lookup_extended(daemon->idle_reposition_notify_ids,
                                   GINT_TO_POINTER(notify_id), &orig_key,
                                   &value)) {
    return;
  }
  ...
  idle_id =
      g_timeout_add_full(G_PRIORITY_LOW, 50,
                         (GSourceFunc)idle_reposition_notification, data, NULL);
  g_hash_table_insert(daemon->idle_reposition_notify_ids,
...
static GdkFilterReturn _notify_x11_filter(GdkXEvent* xevent, GdkEvent* event,
  ...
  if (xev->xany.type == DestroyNotify) {
    g_hash_table_remove(daemon->monitored_window_hash,
  ...
  if (!g_hash_table_lookup_extended(daemon->monitored_window_hash,
                                    GUINT_TO_POINTER(xev->xany.window),
                                    &orig_key, &value)) {
    return GDK_FILTER_CONTINUE;
  }
...
static void _mouse_entered_cb(GtkWindow* nw, GdkEventCrossing* event,
                              NotifyDaemon* daemon) {
  NotifyTimeout* nt;
  guint id;
...
static void _mouse_exitted_cb(GtkWindow* nw, GdkEventCrossing* event,
                              NotifyDaemon* daemon) {
  if (event->detail == GDK_NOTIFY_INFERIOR) {
    return;
  }
  ...
  NotifyTimeout* nt =
      (NotifyTimeout*)g_hash_table_lookup(daemon->notification_hash, &id);

  nt->paused = FALSE;
}

static gboolean _is_expired(gpointer key, NotifyTimeout* nt,
...
static gboolean _check_expiration(NotifyDaemon* daemon) {
  ...
  g_hash_table_foreach_remove(daemon->notification_hash, (GHRFunc)_is_expired,
...
static void _calculate_timeout(NotifyDaemon* daemon, NotifyTimeout* nt,
                               int timeout) {
  if (timeout == 0) {
    ...
    usec = (gint64)timeout *
           G_TIME_SPAN_MILLISECOND; /* convert from msec to usec */

    nt->expiration = usec + g_get_monotonic_time();

    if (daemon->timeout_source == 0) {
      daemon->timeout_source =
          g_timeout_add(100, (GSourceFunc)_check_expiration, daemon);
    }
  }
}
...
static NotifyTimeout* _store_notification(NotifyDaemon* daemon, GtkWindow* nw,
  ...
  g_hash_table_insert(daemon->notification_hash, g_memdup2(&id, sizeof(guint)),
                      nt);
#else
  g_hash_table_insert(daemon->notification_hash, g_memdup(&id, sizeof(guint)),
                      nt);
#endif
  remove_exit_timeout(daemon);
...
static GdkPixbuf* _notify_daemon_pixbuf_from_data_hint(GVariant* icon_data) {
  ...
  g_variant_get(icon_data, "(iiibii@ay)", &width, &height, &rowstride,
                &has_alpha, &bits_per_sample, &n_channels, &data_variant);

  expected_len = (gsize)((height - 1) * rowstride +
  ...
  if (expected_len != g_variant_get_size(data_variant)) {
    g_warning("Expected image data to be of length %" G_GSIZE_FORMAT
              " but got a "
              "length of %" G_GSIZE_FORMAT,
              expected_len, g_variant_get_size(data_variant));
    return NULL;
  }
...
static GdkPixbuf* _notify_daemon_pixbuf_from_path(const char* path) {
  ...
  if (!strncmp(path, "file://", 7) || *path == '/') {
    ...
    icon_info = gtk_icon_theme_lookup_icon(theme, path, IMAGE_SIZE,
...
static GdkPixbuf* _notify_daemon_scale_pixbuf(GdkPixbuf* pixbuf,
...
static void window_clicked_cb(GtkWindow* nw, GdkEventButton* button,
                              NotifyDaemon* daemon) {
...
static gboolean screensaver_active(GtkWidget* nw) {
  ...
  proxy = g_dbus_proxy_new_for_bus_sync(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.mate.ScreenSaver",
      "/", "org.mate.ScreenSaver", NULL, &error);
  if (proxy == NULL) {
    g_warning("Failed to get dbus connection: %s", error->message);
    g_error_free(error);
  }

  variant = g_dbus_proxy_call_sync(proxy, "GetActive", g_variant_new("()"),
...
static gboolean fullscreen_window_exists(GtkWidget* nw) {
  ...
  wnck_screen = wnck_screen_get(
      GDK_SCREEN_XNUMBER(gdk_window_get_screen(gtk_widget_get_window(nw))));
  ...
  for (l = wnck_screen_get_windows_stacked(wnck_screen); l != NULL;
       l = l->next) {
...
static Window get_window_parent(GdkDisplay* display, Window window,
                                Window* root) {
  Window parent;
  ...
  result = XQueryTree(GDK_DISPLAY_XDISPLAY(display), window, root, &parent,
                      &children, &nchildren);
...
static void monitor_notification_source_windows(NotifyDaemon* daemon,
  ...
  for (parent = get_window_parent(display, source, &root);
    ...
    g_hash_table_insert(daemon->monitored_window_hash, GUINT_TO_POINTER(parent),
...
static void sync_notification_position(NotifyDaemon* daemon, GtkWindow* nw,
  ...
  result = XGetGeometry(GDK_DISPLAY_XDISPLAY(display), source, &root, &x, &y,
   ...
   */
  gdk_x11_display_error_trap_push(display);
  result = XTranslateCoordinates(GDK_DISPLAY_XDISPLAY(display), source, root, 0,
                                 0, &x, &y, &child);
  if (gdk_x11_display_error_trap_pop(display) || !result) {
    return;
  }
...
static gboolean notify_daemon_notify_handler(
  ...
  if (g_hash_table_size(daemon->notification_hash) > MAX_NOTIFICATIONS) {
    g_dbus_method_invocation_return_error(
        invocation, notify_daemon_error_quark(), 1,
  ...
  sound_enabled =
      g_settings_get_boolean(gsettings, GSETTINGS_KEY_SOUND_ENABLED);
  do_not_disturb =
      g_settings_get_boolean(gsettings, GSETTINGS_KEY_DO_NOT_DISTURB);
  g_object_unref(gsettings);

  /* If we are in do-not-disturb mode, just grab a new id and close the
   * notification */
  if (do_not_disturb) {
  ...
  for (i = 0; actions[i] != NULL; i += 2) {
    if (actions[i + 1] == NULL) {
      g_warning(
          "Label not found for action %s. The protocol specifies that a label "
          "must follow an action in the actions array",
          actions[i]);
    ...
    if (strcasecmp(actions[i], "default")) {
      theme_add_notification_action(nw, actions[i + 1], actions[i],
...
#ifdef HAVE_X11
  if (GDK_IS_X11_DISPLAY(gdk_display_get_default()) && window_xid != None &&
    ...
    if (use_pos_data && !theme_get_always_stack(nw)) {
      ...
      if (g_settings_get_boolean(daemon->gsettings,
        ...
        monitor_id = gdk_display_get_monitor_at_point(
            gdk_screen_get_display(screen), coordinate_x, coordinate_y);
      } else {
        screen = gdk_display_get_default_screen(gdk_display_get_default());
        monitor_id = gdk_display_get_monitor(
            gdk_display_get_default(),
            g_settings_get_int(daemon->gsettings,
...
#endif /* HAVE_X11 */
  /* fullscreen_window is assumed to be false on Wayland, as there is no trivial
   * way to check */
   ...
   */
  if (!nt->has_timeout ||
      (!screensaver_active(GTK_WIDGET(nw)) && !fullscreen_window)) {
    theme_show_notification(nw);
...
static gboolean notify_daemon_close_notification_handler(
  ...
  if (id == 0) {
    g_dbus_method_invocation_return_error(
        invocation, notify_daemon_error_quark(), 100,
...
static gboolean notify_daemon_get_capabilities(
  ...
  notify_daemon_notifications_complete_get_capabilities(
      object, invocation, (const gchar* const*)g_variant_dup_strv(value, NULL));
...
static gboolean notify_daemon_get_server_information(
  ...
  notify_daemon_notifications_complete_get_server_information(
      object, invocation, "Notification Daemon", "MATE", PACKAGE_VERSION,
      "1.1");
  return TRUE;
}
