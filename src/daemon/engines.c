...
static ThemeEngine* load_theme_engine(const char* name) {
  ...
  if (engine->module == NULL) goto error;
...
#define BIND_OPTIONAL_FUNC(name) \
  ...
  if (!engine->theme_check_init(NOTIFICATION_DAEMON_MAJOR_VERSION,
    ...
    g_warning(
        "Theme doesn't work with this version of mate-notification-daemon");
    goto error;
  }
...
static void theme_changed_cb(GSettings* settings, gchar* key,
                             gpointer user_data) {
  ...
  if (active_engine == NULL) {
    ...
    if (theme_prop_notify_id == 0) {
      theme_prop_notify_id =
          g_signal_connect(gsettings, "changed::" GSETTINGS_KEY_THEME,
...
void theme_set_notification_arrow(GtkWindow* nw, gboolean visible, int x,
                                  int y) {
  ThemeEngine* engine = g_object_get_data(G_OBJECT(nw), "_theme_engine");
